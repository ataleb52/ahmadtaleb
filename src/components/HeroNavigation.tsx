...
export function HeroNavigation({ 
  ...
  const handleCardClick = (key: string, href?: string) => {
    if (key === 'about') {
      setIsBioPanelOpen(prev => !prev); // Toggle the panel
    } else if (href && onNavLinkClick) {
      setIsBioPanelOpen(false); // Close panel if navigating elsewhere
      onNavLinkClick(href); 
    }
  };
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        // isBioPanelOpen ? "opacity-50 pointer-events-none" : "opacity-100" // Optional: dim background when panel is open
      )}>
