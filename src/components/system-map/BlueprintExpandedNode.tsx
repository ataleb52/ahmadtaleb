...
type BlueprintExpandedNodeData = {
  label: string;
  description?: string;
...
export function BlueprintExpandedNode({ data }: NodeProps<BlueprintExpandedNodeData>) {
  ...
  return (
    <div 
      className={`p-3 rounded-sm bg-slate-800 border ${
        data.selected ? 'border-blue-400 shadow-[0_0_15px_rgba(2,132,199,0.5)]' : 'border-blue-600'
    ...
    >
      ...
      {isExpanded && data.variants && data.variants.length > 0 && (
        <div className="mt-3">
          <div className="text-xs font-medium text-slate-200 mb-2">Variants</div>
          <div className="flex flex-wrap gap-1">
            {data.variants.map((variant: VariantOption) => (
              <button
                key={variant.id}
      ...
      {isExpanded && data.layers && (
        <div className="mt-4 perspective-[800px] transform-gpu">
          ...
          <div className="relative py-4">
            {data.layers.map((layer: Layer, index: number) => (
              <div 
                key={layer.id}
              ...
              >
                ...
                {/* CSS Properties */}
                <div className="flex flex-wrap gap-1">
                  {layer.cssProperties.map((prop: string, i: number) => (
